...
if (isset($_GET["class"])) {
  ...
  $mode = $_GET["mode"];
  $sqlCount = "SELECT COUNT(*) as count FROM rent WHERE mode='$mode' AND valid=1";
  $resultCount = $conn->query($sqlCount);
  $row = $resultCount->fetch_assoc();
  $productTotalCount = $row['count'];
} else {
...
} elseif (isset($_GET["class"]) && isset($_GET["p"])) {
  $class = $_GET["class"];
  $p = isset($_GET["p"]) ? $_GET["p"] : 1;
  $startIndex = ($p - 1) * $perPage;
  $sql = "SELECT * FROM rent WHERE class_id= '$class' AND valid=1 LIMIT $startIndex, $perPage";
} elseif (isset($_GET["mode"]) && isset($_GET["p"])) {
  $mode = $_GET["mode"];
  $p = isset($_GET["p"]) ? $_GET["p"] : 1;
  $startIndex = ($p - 1) * $perPage;
  $sql = "SELECT * FROM rent WHERE mode= '$mode' AND valid=1 LIMIT $startIndex, $perPage";
} elseif (isset($_GET["p"])) {
  $p = $_GET["p"];
  $startIndex = ($p - 1) * $perPage;
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        ...
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder mb-0">租借商品列表</h6>
        </nav>
      </div>
    </nav>
    <div class="container">
      ...
      <!-- Modal -->
      <div class="container">
        <h2 class=text-center>租借商品列表</h2>
        <form action="rent_products.php">
          <div class="input-group  mb-3">
            <?php if (isset($_GET["search"])) : ?>
              <div class="col-auto ">
                <a name="" id="" class="btn btn-primary my-3" href="rent_products.php" role="button"><i class="fa-solid fa-arrow-left fa-fw"></i></a>
              </div>
            <?php endif; ?>
            <input type="hidden" name="order" value="<?= isset($_GET["order"]) ? $_GET["order"] : 1 ?>">
            <input type="hidden" name="p" value="<?= isset($_GET["p"]) ? $_GET["p"] : 1 ?>">
            <input type="search" class="form-control my-3" placeholder="請輸入要搜尋的商品關鍵字" aria-describedby="button-addon2" name="search" value="<?= isset($_GET["search"]) ? $_GET["search"] : '' ?>">
            <button class="btn btn-primary my-3" type="submit" id="button-addon2"><i class="fa-solid fa-magnifying-glass"></i></button>
          </div>
        </form>
        ...
        <?php if (!isset($_GET["search"])) : ?>
          <div class="d-flex justify-content-between">
            <div class="input-group mb-3">
              <h4 class="pt-1 mt-2">排序方式:</h5>
                <button style="height:45px;" class="ms-2 mt-2 rounded-pill btn btn-outline-secondary me-auto dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                  <?php if ($order == 1) {
                    echo "編碼由小到大";
                  } elseif ($order == 2) {
                    echo "編碼由大到小";
            ...
            <h4 class="pt-2 me-2 text-nowrap">共<?= $productTotalCount ?> 個商品
            </h4>
        ...
        <?php if ($productCount > 0) : ?>
          <div class="d-flex justify-content-between">
            <div class="py-2">
              ...
              <ul class="nav nav-tabs">
                <h4 class="pt-1 me-2">分類:</h4>
                <li class="nav-item">
                  <a class="nav-link <?php if (!isset($_GET["class"]))
                                        echo "active"; ?>
          " aria-current="page" href="rent_products.php">全部</a>
                ...
                foreach ($rowsproductInfo as $productinfo) : ?>
                  <li class="nav-item">
                    <a class="nav-link <?php if (isset($_GET["class"]) && $_GET["class"] == $productinfo["class"])
                                          echo "active";
                                        ?>" aria-current="page" href="rent_products.php?class=<?= $productinfo["class"] ?>&order=<?php $other = isset($_GET["order"]) ? $_GET["order"] : 1;
          ...
          " aria-current="page" href="rent_products.php">全部</a>
                ...
                <li class="nav-item">
                  <a class="nav-link <?php if (isset($_GET["mode"]) && $_GET["mode"] == "上架") {
                                        echo "active";
                                      } ?>
          " aria-current="page" href="rent_products.php?mode=上架&order=<?= isset($_GET["order"]) ? $_GET["order"] : 1; ?>&p=<?= isset($_GET["p"]) ? $_GET["p"] : 1; ?>">上架中</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link  <?php if (isset($_GET["mode"]) && $_GET["mode"] == "下架") {
                                        echo "active";
                                      } ?>
          " aria-current="page" href="rent_products.php?mode=下架&order=<?= isset($_GET["order"]) ? $_GET["order"] : 1; ?>&p=<?= isset($_GET["p"]) ? $_GET["p"] : 1; ?>">已下架</a>
                </li>
              </ul>
              ...
              ?>
                <tr class="text-center">
                  ...
                  <td class="text-nowrap">
                    <a class="mx-2" role="button" href="rent_product-edit.php?id=<?= $rent["id"] ?>"><i class="fa-solid fa-pen-to-square" style="color: #FFD43B;"></i></a>
                ...
                <div class="modal fade" id="exampleModal<?= $rent["id"] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              ...
              <?php if (isset($_GET["search"])) : ?>
                <?php for ($i = 1; $i <= $pageCount; $i++) : ?>
                  <li class="page-item <?php if ($i == $p)
                    ...
                    <a class="page-link" href="rent_products.php?search=<?= $search ?>&order=<?= isset($_GET["order"]) ? $_GET["order"] : 1 ?>&p=<?= $i ?>">
                      <?= $i ?>
                    </a>
                  </li>
                <?php endfor; ?>
              ...
              <?php elseif (isset($_GET["class"])) : ?>
                <?php for ($i = 1; $i <= $pageCount; $i++) : ?>
                  <li class="page-item <?php if ($i == $p)
                    ...
                    <a class="page-link" href="rent_products.php?class=<?= $class ?>&order=<?= isset($_GET["order"]) ? $_GET["order"] : 1 ?>&p=<?= $i ?>">
                      <?= $i ?>
                    </a>
                  </li>
                <?php endfor; ?>
              <?php elseif (isset($_GET["mode"])) : ?>
                <?php for ($i = 1; $i <= $pageCount; $i++) : ?>
                  <li class="page-item <?php if ($i == $p)
                                          echo "active"; ?>">
                    <?php if (isset($_GET["mode"])) {
                    ...
                    <a class="page-link" href="rent_products.php?mode=<?= $mode ?>&order=<?= isset($_GET["order"]) ? $_GET["order"] : 1 ?>&p=<?= $i ?>">
                      <?= $i ?>
                    </a>
                  </li>
                <?php endfor; ?>
